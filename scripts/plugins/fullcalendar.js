(function ($) {
  ...
  option = {
    ...
    eventClick: function(event, jsEvent) {
      ...
      var event_date = '<div><i class="fa fa-fw fa-calendar-o text-muted"></i> '+moment(event.start).format("YYYY-MM-DD")+'</div>';
      var event_time = '<div><i class="fa fa-fw fa-clock-o text-muted"></i> '+moment(event.start).format("h:mma")+ ' - '+moment(event.end).format("h:mma")+'</div>';
      var participant = ''
  ...
  var init = function(){
    $.ajax('scripts/plugins/dist/horario.json').done(function(data){
      $.each(data, function(index,item){
        item.start = moment().add(index,'d').add(index,'h');
        item.fecha_renta = moment().add(index,'d')
        item.end = moment().add(index,'d');
